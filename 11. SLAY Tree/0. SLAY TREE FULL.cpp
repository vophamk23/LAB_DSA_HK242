...
public:
    ...
    /**
     * Phương thức chèn một giá trị mới vào cây Splay
     *
     * Thuật toán:
    ...
    void insert(int val)
    {
        // Trường hợp đặc biệt: cây rỗng
        if (!root)
        {
            root = new Node(val); // Tạo node mới làm root
            return;
        }
        ...
        while (curr)
        {
            parent = curr; // Lưu node hiện tại làm parent
            ...
            {
                curr = curr->pRight; // Đi sang cây con phải nếu giá trị lớn hơn hoặc bằng
            }
        }

        // Tạo node mới với parent đã xác định
        Node *newNode = new Node(val, nullptr, nullptr, parent);

        // Gán node mới vào vị trí thích hợp
        if (val < parent->val)
        {
            parent->pLeft = newNode; // Gán làm con trái nếu giá trị nhỏ hơn
        }
        else
        {
            parent->pRight = newNode; // Gán làm con phải nếu giá trị lớn hơn hoặc bằng
        }

        // Splay node mới lên làm gốc - đây là điểm khác biệt của Splay Tree so với BST thông thường
        splay(newNode);
    }
